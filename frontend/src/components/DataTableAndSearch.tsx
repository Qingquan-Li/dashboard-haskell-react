...
// Define the type for the individual data item
type Person = {
  firstName: string;
  lastName: string;
  companyName: string;
...
export default function DataTableAndSearch() {
  ...
  const fetchData = async () => {
    const result = await axios.get<Person[]>(RootAPIURL + '/data');
    setData(result.data);
  };
  useEffect(() => {
    fetchData();
  ...
  return (
    <div className="min-h-screen flex flex-col p-4">
      ...
      {data.length === 1 && searchField === 'firstName' ? (
        <div className="flex items-center">
          <img className="mr-4" src="https://raw.githubusercontent.com/Qingquan-Li/files/main/us-data-demo/user.png" alt="User Icon" />
          <div>
            <h2 className="text-xl font-semibold">{data[0].firstName} {data[0].lastName}</h2>
            <p className="text-gray-700">Company Name: {data[0].companyName}</p>
            ...
            <p className="text-gray-700">Zip: {data[0].zipCode}</p>
      ...
      ) : data.length === 1 && searchField === 'companyName' ? (
        <div>
          ...
          <div>
            <h2 className="text-xl font-semibold">{data[0].companyName}</h2>
            <p className="text-gray-700">Address: {data[0].address}</p>
          </div>
        </div>
      ) : (
          ...
          <tbody>
            {data.map(person => (
              <tr key={person.email} className="hover:bg-gray-200">
                <td className="border-2 border-gray-400 p-2">{person.firstName}</td>
                <td className="border-2 border-gray-400 p-2">{person.lastName}</td>
                <td className="border-2 border-gray-400 p-2">{person.companyName}</td>
                ...
                <td className="border-2 border-gray-400 p-2">{person.zipCode}</td>
